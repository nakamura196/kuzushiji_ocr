/*
 * KuroNet Text Editor v1.0 (Powered by IIIF Curation Viewer)
...
var IIIFCurationViewer = function(config) {
    ...
    var CUSTOMIZED_FOR_CHAR_MARKER_EDIT = true; //カスタマイズ箇所の目安
    //カスタマイズなし版と挙動が異なる箇所はif文で切り分けを行ったが、
    //カスタマイズ版独自の変数宣言、関数宣言についてはコメントで示すに留めている。

    var APP_NAME = 'KuroNet Text Editor'; //CUSTOMIZED_FOR_CHAR_MARKER_EDIT
    var VERSION  = '1.0.1+20201021'; //CUSTOMIZED_FOR_CHAR_MARKER_EDIT
    ...
    //CUSTOMIZED_FOR_CHAR_MARKER_EDIT start
    var TEXT_GRANULARITY = {
        GLYPH: 0,
        WORD: 1,
        LINE: 2,
        PARAGRAPH: 3,
    ...
    var textGranularityLevelLabels = [
        [{ '@language': 'en', '@value': 'Glyph' }, { '@language': 'ja', '@value': '文字' }],
        [{ '@language': 'en', '@value': 'Word' }, { '@language': 'ja', '@value': '単語' }],
        [{ '@language': 'en', '@value': 'Line' }, { '@language': 'ja', '@value': '行' }],
        [{ '@language': 'en', '@value': 'Paragraph' }, { '@language': 'ja', '@value': '段落' }],
        [{ '@language': 'en', '@value': 'Block' }, { '@language': 'ja', '@value': '段' }],
        [{ '@language': 'en', '@value': 'Page' }, { '@language': 'ja', '@value': 'ページ' }]
    ];
    var textGranularityLevelList = [TEXT_GRANULARITY.GLYPH, TEXT_GRANULARITY.LINE];
    var uiTextGranularityLevel = TEXT_GRANULARITY.GLYPH;

    var transcriptionDialogTransform = null;
    var transcriptionDialogSize = null;
    //CUSTOMIZED_FOR_CHAR_MARKER_EDIT end

    var err;
    var isInTransition = false;

    var tilesizeFallback = false;
    ...
    var defaultConfig = {
        ...
        transcriptionDialog: { //CUSTOMIZED_FOR_CHAR_MARKER_EDIT
            //整数＋単位で指定（ピクセル単位であれば'px'、マップの大きさに対する相対指定であれば'%'）
            defaultWidth: '10%',
            defaultHeight: '50%'
        }
    };
    if (CUSTOMIZED_FOR_CHAR_MARKER_EDIT) {
        defaultConfig.doc.aboutUrl = 'http://codh.rois.ac.jp/software/kuronet-text-editor/';
    }
    ...
    function configure(config, defaultConfig) {
        ...
        if ($.isPlainObject(config)) {
            ...
            if (CUSTOMIZED_FOR_CHAR_MARKER_EDIT) {
                if ($.isPlainObject(config.transcriptionDialog)) {
                    if ($.type(config.transcriptionDialog.defaultWidth) === 'string') {
                        conf_.transcriptionDialog.defaultWidth = config.transcriptionDialog.defaultWidth;
                    }
                    if ($.type(config.transcriptionDialog.defaultHeight) === 'string') {
                        conf_.transcriptionDialog.defaultHeight = config.transcriptionDialog.defaultHeight;
                    }
                }
            }
        }
        ...
        $.getJSON(curationUrl, function(curation_) {
            if (isValidCurationFalseTrue(curation_)) {
                ...
                if (getMode() === 'annotation' || curation_.viewingHint === 'annotation') {
                    ...
                    setMode('annotation');
                    if (CUSTOMIZED_FOR_CHAR_MARKER_EDIT) {
                        //通常のキュレーション編集は不可だが、文字マーカーの接続情報をエクスポートするためのUIとして
                        //キュレーションリスト画面は利用可能とする。
                    } else {
                        enableCurationEdit = false;
                    }
    ...
    function processManifests(manifests, manifestUrls, bookParams, optPage) {
        ...
        if (enableCurationEdit) {
            ...
            if (CUSTOMIZED_FOR_CHAR_MARKER_EDIT && getMode() === 'annotation') {
                $('#fav_star_link').hide();
            }
        } else {
            $('#curation_nav').hide();
        ...
        if (CUSTOMIZED_FOR_CHAR_MARKER_EDIT && getMode() === 'annotation') {
            $('#curation_list_clear').html('<span class="glyphicon glyphicon-remove"></span> ' + ((lng !== 'ja') ? 'Clear All Reading Order' : '読み順を全てクリア'))
                .attr('title', (lng !== 'ja') ? 'Clear all reading order' : '読み順を全てクリア');
        } else {
    ...
    function refreshPage() {
        ...
        var INTER_MARKER_LINE_LAYER_NAME = 'interMarkerLine'; //CUSTOMIZED_FOR_CHAR_MARKER_EDIT
        var isFirstTime;
        var width = getCanvasWidth(page);
        var tileSize = (width >= TILE_SIZE_PREFERRED && tilesizeFallback === false) ? TILE_SIZE_PREFERRED : TILE_SIZE_DEFAULT;
        ...
        if (getCanvasImageInfoUrl(page)) {
            iiif = L.tileLayer.iiif(getCanvasImageInfoUrl(page), {
                tileSize: (getCanvasImageComplianceLevel(page) && tilesizeFallback === false) ? tileSize : void 0,
                fitBounds: fitBounds,
        ...
        if (tileSize > TILE_SIZE_DEFAULT) {
            ...
            iiif.on('tileerror', function() {
                isTileLoadDone = true;
                map.spin(false);
                if (tilesizeFallback === false) {
                    tilesizeFallback = true;
                    if (iiif.hasImageAPIservice && tileSize > TILE_SIZE_DEFAULT) {
                        map.remove();
                        map = void 0;
                        refreshPage();
                    }
        ...
        var hasAnnotationShown = false; //CUSTOMIZED_FOR_CHAR_MARKER_EDIT
        iiif.on('load', function() {
            ...
            //CUSTOMIZED_FOR_CHAR_MARKER_EDIT start
            var setUiTextGranularityLevelFuncs = [];
            function setUiTextGranularityLevel(level) {
                return function() {
                    if (level !== uiTextGranularityLevel) {
                        $('#text_granularity_level_' + level).parent().click(); //暫定
                    }
                };
            }
            for (var idx in textGranularityLevelList){
                var lv = textGranularityLevelList[idx];
                setUiTextGranularityLevelFuncs[lv] = setUiTextGranularityLevel(lv);
            }
            //CUSTOMIZED_FOR_CHAR_MARKER_EDIT end
            if (iiif.x && iiif.y) {
                var bounds;
                            ...
                            if ($.isPlainObject(annotation.resource) && $.isPlainObject(annotation.resource.marker)) {
                                ...
                                if (markerType) {
                                    ...
                                    if (CUSTOMIZED_FOR_CHAR_MARKER_EDIT) {
                                        if (markerType == 1) {
                                            //文字マーカー
                                            markerOptions.markerId_next = marker.next || null;
                                            markerOptions.markerId_prev = marker.prev || null;
                                            markerOptions.markerId_next_line = marker.next_line || null;
                                            markerOptions.markerId_prev_line = marker.prev_line || null;
                                        }
                                        if (markerType == 1) {
                                            var contextmenuItems = [];
                                            for (idx in textGranularityLevelList){
                                                var lv_ = textGranularityLevelList[idx];
                                                contextmenuItems.push({
                                                    text: ((lng !== 'ja') ? 'Reading Order: ' : '読み順設定対象：') + getPropertyValueI18n(textGranularityLevelLabels[lv_]),
                                                    callback: setUiTextGranularityLevelFuncs[lv_]
                                                });
                                            }
                                            markerOptions.contextmenu = true;
                                            markerOptions.contextmenuItems = contextmenuItems;
                                        }
                                    }
                                    var marker_;
                                    ...
                                    if (CUSTOMIZED_FOR_CHAR_MARKER_EDIT) {
                                        if (markerType == 1) {
                                            //文字マーカー
                                            marker_.on('mousedown', startAppendTextToTranscriptionDialog);
                                            marker_.on('mouseover', appendTextToTranscriptionDialog);
                                        }
                                    }
                    ...
                    if (markers.length > 0) {
                        ...
                        if (CUSTOMIZED_FOR_CHAR_MARKER_EDIT) {
                            ...
                            for (var k = 0; k < markers.length; k++) {
                                var marker___ = markers[k];
                                if (marker___.options.markerId_prev) {
                                    setupInterMarkerLine(marker___, marker___.options.markerId_prev, TEXT_GRANULARITY.GLYPH);
                                }
                                if (marker___.options.markerId_prev_line) {
                                    setupInterMarkerLine(marker___, marker___.options.markerId_prev_line, TEXT_GRANULARITY.LINE);
                                }
                            }
                            hasAnnotationShown = true;
                        }
                        ...
                        if (CUSTOMIZED_FOR_CHAR_MARKER_EDIT) {
                            //順序設定対象指定パネル
                            var $uiTextGranularityLevel = $('<div>').attr('id', 'text_granularity_level').addClass('btn-group').attr('data-toggle', 'buttons');
                            $uiTextGranularityLevel.append('<h6>' + ((lng !== 'ja') ? 'Reading Order' : '読み順設定対象') + '</h6>');
                            for (var m in textGranularityLevelList){
                                i = textGranularityLevelList[m];
                                var $label_ = $('<label>').addClass('btn btn-default btn-sm');
                                if (i === uiTextGranularityLevel) {
                                    $label_.addClass('active');
                                }
                                var $input_ = $('<input>').attr('type', 'radio').attr('name', 'options')
                                    .attr('value', i).attr('id', 'text_granularity_level_' + i);
                                var label = getPropertyValueI18n(textGranularityLevelLabels[i]);
                                $label_.append($input_).append(label);
                                $uiTextGranularityLevel.append($label_);
                            }
                            var slidersOption_ = {
                                position: 'topleft',
                                footer: $uiTextGranularityLevel.prop('outerHTML')
                            };
                            L.control.sliders([], slidersOption_).addTo(map);
                            $('#text_granularity_level input[type=radio]').change(function() {
                                uiTextGranularityLevel = parseInt(this.value, 10);
                            });
                        }
                    }
                }
        ...
        //CUSTOMIZED_FOR_CHAR_MARKER_EDIT start
        var touchMouseEmurationLastMarkerId = null;
        function touchMouseEmulation(e, callback, threshold) {
            if (e && e.originalEvent && e.originalEvent.touches && $.isFunction(callback) && iiif.x && iiif.y) {
                var touch = e.originalEvent.touches[0];
                var ev = {
                    clientX: touch.clientX,
                    clientY: touch.clientY
                };
                var minLatLng = map.mouseEventToLatLng(ev);
                var maxCanvasPoint = L.point(iiif.x, iiif.y);
                ...
                if (maxCanvasLatLng.lng > 0 && maxCanvasLatLng.lat < 0) {
                    ...
                    map.eachLayer(function(layer) {
                        if (layer.options && layer.options.name === ANNOTATION_LAYER_NAME) {
                            var latLng = layer.getLatLng();
                            var diffLat = Math.abs(minLatLng.lat - latLng.lat);
                            var diffLng = Math.abs(minLatLng.lng - latLng.lng);
                            var dist = Math.sqrt(diffLat * diffLat + diffLng * diffLng);
                            if (layer.options.markerId !== touchMouseEmurationLastMarkerId &&
                                diffLat < threshold && diffLng < threshold && dist < minDist) {
                                minDist = dist;
                                target = layer;
                            }
                        }
                    ...
                    if (target) {
                        touchMouseEmurationLastMarkerId = target.options.markerId;
                        ev.target = target;
                        callback(ev);
                    }
                }
        ...
        if ('ontouchstart' in window && CUSTOMIZED_FOR_CHAR_MARKER_EDIT && getMode() === 'annotation') {
            $('.leaflet-marker-pane').on({
                'touchstart' : function(e) {
                    touchMouseEmulation(e, startAppendTextToTranscriptionDialog, 15);
                },
                'touchmove': function(e) {
                    touchMouseEmulation(e, appendTextToTranscriptionDialog, 5);
                },
                'touchend touchcancel': function() {
                    touchMouseEmurationLastMarkerId = null;
                    map.dragging.enable();
                    if (hasAnnotationShown) {
                        updateCurationCanvasMetadata();
                    }
                }
            });
        }
        var markerId_prev = null;
        var markerId_prev_line = null;
        var is_last_action_del_line = false;
        var transcription_buffer = '';
        function showTranscriptionDialog(transcription) {
            function unitToPixel(exp, max, default_) {
                var pixel = default_;
                if (exp) {
                    var match = exp.toLowerCase().match(/^([0-9]+)(px|%)?$/);
                    if (match) {
                        pixel = parseInt(match[1], 10);
                        if (match.length > 2) {
                            var unit = match[2];
                            if (unit === '%') {
                                pixel = pixel * max / 100;
            ...
            var defaultWidth = conf.transcriptionDialog && conf.transcriptionDialog.defaultWidth;
            var dialogWidth = unitToPixel(defaultWidth, mapWidth - 20, mapWidth / 2);
            var dialogMinWidth = 56;
            if (windowWidth < 767) {
                if (dialogWidth < dialogMinWidth) {
                    dialogWidth = dialogMinWidth;
                }
            }
            var defaultHeight = conf.transcriptionDialog && conf.transcriptionDialog.defaultHeight;
            var dialogHeight = unitToPixel(defaultHeight, mapHeight - 20, 95);
            var dialogMinHeight = 56;
            ...
            var options = {
                ...
                minSize: [dialogMinWidth, dialogMinHeight],
                maxSize: [mapWidth, mapHeight - 20]
            };
            if (transcriptionDialogTransform) {
                options.transform = [transcriptionDialogTransform.x, transcriptionDialogTransform.y];
            }
            if (transcriptionDialogSize) {
                options.size = transcriptionDialogSize;
            }
            var box = L.control.dialog(options).addTo(map);
            $('.leaflet-control-dialog').hide();
            ...
            $('.leaflet-control-dialog').fadeIn(200);
            box.setContent(transcription);
            return box;
        }
        function startAppendTextToTranscriptionDialog(e) {
            map.dragging.disable();
            if (box) {
                $('.leaflet-control-dialog').fadeOut(400);
            }
            transcription_buffer = '';
            if (!is_last_action_del_line) {
                //直前の動作が連結削除であった場合、連結し漏れた（スキップしてしまった）文字を正しく連結するために
                //連結を削除したものと仮定する。このとき、削除された連結が属していたクラスタの文字マーカーが全て
                //再表示されてしまうと、連結修正作業が分かりにくいので、この場合は再表示しない。
                map.eachLayer(function(layer_) {
                    if (layer_.options && layer_.options.name === ANNOTATION_LAYER_NAME) {
                        //文字マーカー再表示
                        $('.icv-annotation-div-icon > span[data-marker-id="' + layer_.options.markerId + '"]').show();
                    }
                });
            }
            is_last_action_del_line = false;
            appendTextToTranscriptionDialog(e);
        }
        function appendTextToTranscriptionDialog(e) {
            if (!map.dragging.enabled()) {
                if (e && e.target && e.target.options) {
                    if (uiTextGranularityLevel === TEXT_GRANULARITY.GLYPH) {
                        //文字単位
                        var markerId_current = e.target.options.markerId;
                        var $marker = $('.icv-annotation-div-icon > span[data-marker-id="' + markerId_current + '"]');
                        var connected = connectMarker(e.target, markerId_prev, uiTextGranularityLevel); //接続されたか
                        var isValidStartMarker = !markerId_prev && !e.target.options.markerId_next; //接続の始点たりうるか
                        if (connected || isValidStartMarker) {
                            $marker.hide('fade', {duration: 500});
                            if (markerId_current !== markerId_prev) { //フラッタリング防止
                                transcription_buffer += e.target.options.text;
                            }
                        }
                        if (connected || !markerId_prev) {
                            markerId_prev = markerId_current;
                        }
                    } else if (uiTextGranularityLevel === TEXT_GRANULARITY.LINE) {
                        //行単位
                        var headMarker = getHeadMarkerOfLine(e.target);
                        if (headMarker) {
                            var markerId_current_head = headMarker.options.markerId;
                            var connected_ = connectMarker(headMarker, markerId_prev_line, uiTextGranularityLevel); //接続されたか
                            if (connected_ || !markerId_prev_line) {
                                markerId_prev_line = markerId_current_head;
                            }
                        }
                    }
                }
        ...
        //文字マーカー間の連結線オブジェクト（L.polyline）をセットアップ
        function setupInterMarkerLineCore(marker_prev, marker_next, textGranularityLevel) {
            //連結線のコンテクストメニュー
            var deleteReadingOrderText;
            var deleteReadingOrderGroupText;
            if (textGranularityLevel === TEXT_GRANULARITY.GLYPH) {
                deleteReadingOrderText = (lng !== 'ja') ? 'Delete character reading order' : '文字の読み順を削除';
                deleteReadingOrderGroupText = (lng !== 'ja') ? 'Delete character reading order group' : '文字の読み順グループを削除';
            } else if (textGranularityLevel === TEXT_GRANULARITY.LINE) {
                deleteReadingOrderText = (lng !== 'ja') ? 'Delete line reading order' : '行の読み順を削除';
                deleteReadingOrderGroupText = (lng !== 'ja') ? 'Delete line reading order group' : '行の読み順グループを削除';
            }
            var polylineOptions = {
                name: INTER_MARKER_LINE_LAYER_NAME,
                clickable: true,
                //連結線オブジェクトは両端のmarkerIdを保持すればよく、markerId_prev_line、markerId_next_lineなどのプロパティは不要
                markerId_prev: marker_prev.options.markerId,
                markerId_next: marker_next.options.markerId,
                textGranularityLevel: textGranularityLevel,
                contextmenu: true,
                contextmenuItems: [
                    {
                        text: deleteReadingOrderText,
                        callback: function(e) {
                            if (e.relatedTarget && e.relatedTarget.options) {
                                deleteInterMarkerLine(e.relatedTarget);
                            }
                        }
                    },
                    {
                        text: deleteReadingOrderGroupText,
                        callback: function(e) {
                            var lines;
                            var markerId;
                            if (e.relatedTarget && e.relatedTarget.options) {
                                textGranularityLevel = e.relatedTarget.options.textGranularityLevel;
                                //選択した連結線から、順方向に連結を辿り、連結線を削除
                                lines = findInterMarkerLine(e.relatedTarget.options.markerId_next, textGranularityLevel, {start: true});
                                markerId = true;
                                while (markerId && lines.length > 0) {
                                    markerId = lines[0].options.markerId_next;
                                    deleteInterMarkerLine(lines[0]);
                                    lines = findInterMarkerLine(markerId, textGranularityLevel, {start: true});
                                }
                                //選択した連結線から、逆方向に連結を辿り、連結線を削除
                                lines = findInterMarkerLine(e.relatedTarget.options.markerId_prev, textGranularityLevel, {end: true});
                                markerId = true;
                                while (markerId && lines.length > 0) {
                                    markerId = lines[0].options.markerId_prev;
                                    deleteInterMarkerLine(lines[0]);
                                    lines = findInterMarkerLine(markerId, textGranularityLevel, {end: true});
                                }
                                //選択した連結線自身を削除
                                deleteInterMarkerLine(e.relatedTarget);
                            }
            ...
            if (textGranularityLevel === TEXT_GRANULARITY.LINE) {
                polylineOptions.color = '#5500bb';
            }
            function findInterMarkerLine(markerId, textGranularityLevel, options) {
                //markerIdを始点／終点とする連結線オブジェクトを探す
                var lines = [];
                map.eachLayer(function(layer) {
                    if (layer.options && layer.options.name === INTER_MARKER_LINE_LAYER_NAME &&
                        layer.options.textGranularityLevel === textGranularityLevel) {
                        //layer：連結線オブジェクト
                        if (options.start) {
                            if (layer.options.markerId_prev === markerId) {
                                lines.push(layer); //markerIdから始まる連結線
                            }
                        }
                        if (options.end) {
                            if (layer.options.markerId_next === markerId) {
                                lines.push(layer); //markerIdで終わる連結線
                            }
                        }
                    }
                });
            ...
            function deleteInterMarkerLine(line) {
                //連結線の削除時の処理
                var markerId_prev = line.options.markerId_prev;
                var markerId_next = line.options.markerId_next;
                var textGranularityLevel = line.options.textGranularityLevel;
                //連結線オブジェクトの削除
                map.removeLayer(line);
                //連結線で結ばれていた文字マーカーから連結情報を削除
                map.eachLayer(function(layer_) {
                    if (layer_.options && layer_.options.name === ANNOTATION_LAYER_NAME) {
                        var marker = layer_;
                        if (textGranularityLevel === TEXT_GRANULARITY.GLYPH) {
                            if (marker.options.markerId === markerId_prev &&
                                marker.options.markerId_next === markerId_next) { //marker：連結線の始点
                                marker.options.markerId_next = null;
                                is_last_action_del_line = true;
                            }
                            if (marker.options.markerId === markerId_next &&
                                marker.options.markerId_prev === markerId_prev) { //marker：連結線の終点
                                marker.options.markerId_prev = null;
                                is_last_action_del_line = true;
                            }
                            if (marker.options.markerId_prev === null && marker.options.markerId_next === null) {
                                //他と連結していない文字マーカーを再表示
                                $('.icv-annotation-div-icon > span[data-marker-id="' + marker.options.markerId + '"]').show();
                            }
                        } else if (textGranularityLevel === TEXT_GRANULARITY.LINE) {
                            if (marker.options.markerId === markerId_prev &&
                                marker.options.markerId_next_line === markerId_next) {
                                marker.options.markerId_next_line = null;
                            }
                            if (marker.options.markerId === markerId_next &&
                                marker.options.markerId_prev_line === markerId_prev) {
                                marker.options.markerId_prev_line = null;
                            }
                        }
                    }
                });
                if (textGranularityLevel !== uiTextGranularityLevel) {
                    //線を削除した後、続けて線を結ぶことを想定し、読み順設定対象を変更する。
                    $('#text_granularity_level_' + textGranularityLevel).parent().click(); //暫定
                }
            }
            function getMidPoint(from, to, deep, round_side_) {
                //Based on "Leaflet Bezier" (MIT License, Copyright (c) 2017 Supun Praneeth)
                //https://github.com/lifeeka/leaflet.bezier/blob/master/src/leaflet.bezier.js#L154
                var round_side = round_side_ || 'LEFT_ROUND';
                var offset = 3.14;
                if (round_side === 'RIGHT_ROUND') {
                    offset = offset * -1;
                }
                var offsetX = to.lng - from.lng;
                var offsetY = to.lat - from.lat;
                var r = Math.sqrt(Math.pow(offsetX, 2) + Math.pow(offsetY, 2));
                var theta = Math.atan2(offsetY, offsetX);
                var thetaOffset = (offset / (deep ? deep : 4));
                var r2 = (r / 2) / (Math.cos(thetaOffset));
                var theta2 = theta + thetaOffset;
                var midpointX = (r2 * Math.cos(theta2)) + from.lng;
                var midpointY = (r2 * Math.sin(theta2)) + from.lat;
                var midpointLatLng = [midpointY, midpointX];
                return midpointLatLng;
            }
            var latlngs;
            var latlng__0 = L.latLng(marker_prev.options.origCenterLat, marker_prev.options.origCenterLng);
            var latlng__1 = L.latLng(marker_next.options.origCenterLat, marker_next.options.origCenterLng);
            if (textGranularityLevel === TEXT_GRANULARITY.GLYPH) {
                latlngs = [latlng__0, latlng__1];
            } else if (textGranularityLevel === TEXT_GRANULARITY.LINE) {
                var midPoint0 = L.latLng(getMidPoint(latlng__0, latlng__1, 10, 'RIGHT_ROUND'));
                var midPoint1 = L.latLng(getMidPoint(latlng__0, midPoint0, 20, 'RIGHT_ROUND'));
                var midPoint2 = L.latLng(getMidPoint(midPoint0, latlng__1, 20, 'RIGHT_ROUND'));
                var midPoint3 = L.latLng(getMidPoint(latlng__0, midPoint1, 40, 'RIGHT_ROUND'));
                var midPoint4 = L.latLng(getMidPoint(midPoint1, midPoint0, 40, 'RIGHT_ROUND'));
                var midPoint5 = L.latLng(getMidPoint(midPoint0, midPoint2, 40, 'RIGHT_ROUND'));
                var midPoint6 = L.latLng(getMidPoint(midPoint2, latlng__1, 40, 'RIGHT_ROUND'));
                latlngs = [latlng__0, midPoint3, midPoint1, midPoint4, midPoint0, midPoint5, midPoint2, midPoint6, latlng__1];
            }
            var line = L.polyline(latlngs, polylineOptions).on('dblclick', function() {
                //連結の削除時の処理
                deleteInterMarkerLine(this);
            });
            line.showExtremities('dotM');
            line.addTo(map);
        }
        //文字マーカー間の連結線オブジェクト（L.polyline）をセットアップ
        function setupInterMarkerLine(marker, markerId_prev_, textGranularityLevel) {
            var result = false;
            if (markerId_prev_) {
                map.eachLayer(function(layer) {
                    if (layer.options && layer.options.name === ANNOTATION_LAYER_NAME &&
                        layer.options.markerId === markerId_prev_) {
                        setupInterMarkerLineCore(layer, marker, textGranularityLevel);
                        result = true;
                    }
                });
            }
        ...
        //まだ線が引かれていない文字マーカー間に限り、連結し、連結線オブジェクトをセットアップする
        function connectMarker(marker, markerId_prev_, textGranularityLevel) {
            //markerオブジェクトを「C」、markerId_prev_で指定されたオブジェクトを「P」として、「P」→「C」を連結する場合
            var result = false;
            if (marker && marker.options && markerId_prev_) {
                var markerId_current = marker.options.markerId;
                if (markerId_current !== markerId_prev_) { //自分自身との接続は不許可
                    if ((textGranularityLevel === TEXT_GRANULARITY.GLYPH && !marker.options.markerId_prev) ||
                        (textGranularityLevel === TEXT_GRANULARITY.LINE && !marker.options.markerId_prev_line)) {
                        //「C」に手前の文字・行との既存接続がないときのみ
                        map.eachLayer(function(layer) {
                            if (layer.options && layer.options.name === ANNOTATION_LAYER_NAME &&
                                layer.options.markerId === markerId_prev_) { //「P」を発見
                                var marker_prev = layer; //「P」のmarkerオブジェクト
                                if ((textGranularityLevel === TEXT_GRANULARITY.GLYPH && !marker_prev.options.markerId_next) ||
                                    (textGranularityLevel === TEXT_GRANULARITY.LINE && !marker_prev.options.markerId_next_line)) {
                                    //「P」に次の文字・行との既存接続がないときのみ
                                    if (textGranularityLevel === TEXT_GRANULARITY.GLYPH) {
                                        //データ構造上の連結
                                        marker_prev.options.markerId_next = markerId_current;
                                        marker.options.markerId_prev = markerId_prev_;
                                    } else if (textGranularityLevel === TEXT_GRANULARITY.LINE) {
                                        //データ構造上の連結
                                        marker_prev.options.markerId_next_line = markerId_current;
                                        marker.options.markerId_prev_line = markerId_prev_;
                                    }
                                    //連結線オブジェクト（L.polyline）をセットアップ
                                    setupInterMarkerLineCore(marker_prev, marker, textGranularityLevel);
                                    result = true;
                                }
                            }
                        });
        ...
        //ある文字マーカーが属している行の先頭文字マーカーを返す（リストが環をなしているときはnull）
        function getHeadMarkerOfLine(marker, optStartMarkerId) {
            if (marker) {
                if (marker.options.markerId_prev) {
                    var markerId_current = marker.options.markerId;
                    var markerId_prev = marker.options.markerId_prev;
                    var startMarkerId = optStartMarkerId || markerId_current;
                    var layers = map._layers;
                    for (var i in layers) {
                        var layer = layers[i];
                        if (layer.options && layer.options.name === ANNOTATION_LAYER_NAME &&
                            layer.options.markerId === markerId_prev &&
                            layer.options.markerId_next === markerId_current) {
                            var marker_prev = layer; //一つ前のマーカー
                            if (marker_prev.options.markerId === startMarkerId) {
                                //リストが環をなしていて、スタート地点に戻ってきた
                                return null;
                            } else {
                                return getHeadMarkerOfLine(marker_prev, startMarkerId);
                            }
                        }
                    }
                } else {
        ...
        //全ての連結を削除
        function deleteAllInterMarkerLine() {
            //全ての連結線オブジェクトを削除
            map.eachLayer(function(layer_) {
                if (layer_.options && layer_.options.name === INTER_MARKER_LINE_LAYER_NAME) {
                    map.removeLayer(layer_);
                }
            });
            //全てのマーカーオブジェクトから連結情報を削除
            map.eachLayer(function(layer_) {
                if (layer_.options && layer_.options.name === ANNOTATION_LAYER_NAME) {
                    layer_.options.markerId_next = null;
                    layer_.options.markerId_prev = null;
                    layer_.options.markerId_next_line = null;
                    layer_.options.markerId_prev_line = null;
                    //他と連結していない文字マーカーを再表示
                    $('.icv-annotation-div-icon > span[data-marker-id="' + layer_.options.markerId + '"]').show();
                }
            });
        ...
        //現在表示している外部curationの内容を更新する
        //通常は、getBrowsingCurationJson()で取得される内容は、編集による影響を受けないが、
        //CUSTOMIZED_FOR_CHAR_MARKER_EDIT の場合は、更新される。
        function updateCurationCanvasMetadata() {
            var annotations = getCanvasAnnotations(page);
            if (annotations) {
                var markers = [];
                map.eachLayer(function(layer) {
                    if (layer.options && layer.options.name === ANNOTATION_LAYER_NAME && layer.options.markerType === 1) {
                        //layer：文字マーカー
                        markers[layer.options.markerId] = {
                            next: layer.options.markerId_next,
                            prev: layer.options.markerId_prev,
                            next_line: layer.options.markerId_next_line,
                            prev_line: layer.options.markerId_prev_line
                        };
                    }
                });
                for (var i = 0; i < annotations.length; i++) {
                    ...
                    if ($.isPlainObject(annotation.resource) && $.isPlainObject(annotation.resource.marker)) {
                        ...
                        if (marker['@id'] && marker['@type'] === 'dctypes:Image') {
                            ...
                            //文字マーカー
                            var markerId = annotation['@id'] || i + 1;
                            var marker_ = markers[markerId];
                            if (marker_.next) {
                                marker.next = marker_.next;
                            } else {
                                delete marker.next;
                            }
                            if (marker_.prev) {
                                marker.prev = marker_.prev;
                            } else {
                                delete marker.prev;
                            }
                            if (marker_.next_line) {
                                marker.next_line = marker_.next_line;
                            } else {
                                delete marker.next_line;
                            }
                            if (marker_.prev_line) {
                                marker.prev_line = marker_.prev_line;
                            } else {
                                delete marker.prev_line;
                            }
                        }
                    }
                }
        ...
        if (CUSTOMIZED_FOR_CHAR_MARKER_EDIT && getMode() === 'annotation') {
            map.on('dialog:moveend', function(e) {
                transcriptionDialogTransform = L.DomUtil.getPosition(e._container);
            });
            map.on('dialog:resizeend', function(e) {
                transcriptionDialogSize = e.options.size;
            });
            map.on('resize', function(e) {
                ...
                if (pos && 'left' in pos && 'top' in pos) {
                    ...
                    if (-pos.left > e.newSize.x - 20) {
                        needReset = true;
                    }
                }
                if (needReset) {
                    if (transcriptionDialogTransform === null) {
                        transcriptionDialogTransform = {};
                    }
                    transcriptionDialogTransform.x = 0;
                    transcriptionDialogTransform.y = 0;
                    transcriptionDialogSize = null;
                    box.destroy();
                    box = showTranscriptionDialog();
                }
            });
            map.on('mouseup mouseout', function() {
                if (markerId_prev && transcription_buffer !== '') {
                    //線を連結した直後に限る（それ以外のときにマウスの出入りがあっても表示不要）
                    var markers = [];
                    map.eachLayer(function(layer) {
                        if (layer.options && layer.options.name === ANNOTATION_LAYER_NAME &&
                            layer.options.markerId === markerId_prev) {
                            //layer：一つ前のマーカー
                            if (layer.options.markerId_prev === null && layer.options.markerId_next === null) { //連結していない
                                markers.push(layer);
                            }
                        }
                    });
                    if (markers.length > 0) {
                        //ある文字マーカーをクリックして非表示状態（もしくはフェイドアウトアニメーション中）にあるものの、
                        //他の文字と連結せずに終えたときは、当該文字マーカーを再表示
                        $('.icv-annotation-div-icon > span[data-marker-id="' + markerId_prev + '"]').stop(true, true).show();
                    } else {
                        if (box) {
                            $('.leaflet-control-dialog').fadeIn(200);
                            box.setContent(transcription_buffer);
                        } else {
                            box = showTranscriptionDialog(transcription_buffer);
                        }
                    }
                }
                markerId_prev = null;
                markerId_prev_line = null;
                map.dragging.enable();
                if (hasAnnotationShown) {
                    updateCurationCanvasMetadata();
                }
            ...
            //キュレーションリスト画面の「全てクリア」ボタンを、読み順を全てクリアに利用する。
            $('#curation_list_clear').off('click.readingorder');
            $('#curation_list_clear').on('click.readingorder', function() {
                deleteAllInterMarkerLine(); //読み順を全てクリア
    ...
    function updateCurationListUrl() {
        ...
        if (CUSTOMIZED_FOR_CHAR_MARKER_EDIT && getMode() === 'annotation') {
            $newUrlElem = $('<div>').html((lng !== 'ja') ? 'The edited reading order is reflected in the curation data.' : '編集した読み順がキュレーションデータに反映されます。');
        }
